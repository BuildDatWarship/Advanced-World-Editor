...
def earthlike(height, plate_mask, world_diameter_km, cdf_factor=1.0,
    ...
    # The DC component can explode when spectral_slope is negative which leads
    # to checkerboard artifacts in the heightmap.  Setting it to zero keeps the
    # overall mean in a safe range before the inverse transform.
    spec[0, 0] = 0.0

    shaped = np.real(ifft2(spec))
    shaped = normalize_map(shaped)
